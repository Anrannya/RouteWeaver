# -*- coding: utf-8 -*-
"""Build rest_kb_data_1320_1419.py from hand-authored fact chunks."""
...
OUT = os.path.join(TOOLS, "rest_kb_data_1320_1419.py")
ALLOWED = {
...
def validate(hand: dict) -> None:
    ...
    missing = [q for q in range(1320, 1420) if q not in hand]
    if missing:
        raise SystemExit(f"Missing qids: {missing[:10]} ... total {len(missing)}")

    extra = [q for q in hand if q < 1320 or q > 1419]
    if extra:
        raise SystemExit(f"Extra qids outside range: {extra[:10]}")

    for qid in range(1320, 1420):
...
def render(hand: dict) -> str:
    lines = [
        "# -*- coding: utf-8 -*-",
        '"""Eval-style rest KB for qids 1320-1419 (100 questions, 5 facts each)."""',
        "",
