# -*- coding: utf-8 -*-
"""Build rest_kb_data_2020_2119.py from hand-authored fact chunks."""
...
OUT = os.path.join(TOOLS, "rest_kb_data_2020_2119.py")
ALLOWED = {
...
def validate(hand: dict) -> None:
    ...
    missing = [q for q in range(2020, 2120) if q not in hand]
    if missing:
        raise SystemExit(f"Missing qids: {missing[:10]} ... total {len(missing)}")

    extra = [q for q in hand if q < 2020 or q > 2119]
    if extra:
        raise SystemExit(f"Extra qids outside range: {extra[:10]}")

    for qid in range(2020, 2120):
...
def render(hand: dict) -> str:
    lines = [
        "# -*- coding: utf-8 -*-",
        '"""Eval-style rest KB for qids 2020-2119 (100 questions, 5 facts each)."""',
        "",
...
def main() -> None:
    hand: dict = {}
    chunk_paths = [
        os.path.join(TOOLS, "_kb_hand_2020_2069.py"),
        os.path.join(TOOLS, "_kb_hand_2070_2119.py"),
    ]
