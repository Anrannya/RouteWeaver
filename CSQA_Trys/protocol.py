"""Canonical execution helpers for CSQA DoT records.

The released ``step2In_csqa_last.json`` contains nine records whose serialized
``depths`` omit a boundary step even though the step and model assignment are
present.  This module repairs only those omitted boundary nodes, using the
decomposer's numbered order, and applies the same deterministic protocol to
every baseline and augmentation branch.
"""
...
def canonical_depths(record: dict[str, Any]) -> dict[int, list[str]]:
    serialized = {
        int(depth): sorted(step_ids, key=step_number)
        for depth, step_ids in record["depths"].items()
    }
    expected = {int(step_id) for step_id in record["steps_dict"]}
    present = {step_number(step_id) for layer in serialized.values() for step_id in layer}
    if present == expected:
    ...
    if unknown:
        raise ValueError(f"DAG contains unknown steps: {sorted(unknown)}")

    edges = {tuple(map(int, edge)) for edge in record.get("int_edges", [])}
    # The known corruptions are omitted first/last boundary nodes.  Connect an
    # omitted node to its immediate numbered neighbour so it cannot be silently
    # treated as an unrelated parallel task.
    ...
    if visited != len(expected):
        raise ValueError("CSQA dependency graph contains a cycle")
    return layers
